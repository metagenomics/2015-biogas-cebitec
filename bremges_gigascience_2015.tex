...
%
\parttitle{Findings}
We report the first deeply sequenced metagenome and metatranscriptome of a biogas-producing microbial community from an agricultural production-scale biogas plant. Our metagenome assembly reconstructed most genes involved in the methane metabolism, indicating sufficient coverage for most downstream analyses.
%a very brief preview of the data type(s) produced, the methods used, and information relevant to data validation
%
\parttitle{Conclusions}
Sequenced at least one order of magnitude deeper than previous studies, our metagenome data will enable novel insights into community composition and the genetic potential of important community members. Moreover, mapping of transcripts to reconstructed genome sequences will enable identification of active metabolic pathways in target organisms.
...
Aliquots of $20\,g$ of the fermentation sample were used for total community DNA preparation as described previously \cite{Schlueter2008}. RAN extraction was done at \emph{vertis}, using some protocol\todo{check.}.
...
On Illumina's Genome Analyzer IIx system, we sequenced further metatranscriptome library with an average insert size of 130nt and 380nt, respectively, applying the Paired-End DNA Sample Preparation Kit (Illumina Inc.) as described by the manufacturer and generating $2 \times 160\,bp$ paired-end reads.\todo[inline]{search the damn transcript to double-check Irena's info - seems wrong to me}
...
\end{tabular}
\label{tReads}
\todo[inline]{Move insert size metrics from Picardtools to another table, add mapping statistics}
\end{table}
