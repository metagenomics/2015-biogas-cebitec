...
The production of biogas takes place under anaerobic conditions and involves microbial decomposition of organic matter. Most of the participating microbes still have to be considered as unknown and non-cultivable. Accordingly, metagenome sequencing currently is the method of choice to obtain insights into community composition and the genetic repertoire.
%a presentation of the interest or relevance of these data for the broader community
%
\parttitle{Findings}
Here, we report the first deeply sequenced metagenome and metatranscriptome of a complex biogas-producing microbial community from an agricultural production-scale biogas plant. We assembled the metagenome and reconstructed most genes involved in the methane metabolism, a key pathway involving methanogenesis performed by methanogenic \emph{Archaea}. This exemplary result indicates sufficient sequencing coverage for most downstream analyses.
...
SAM: Sequence Alignment/Map format, a generic format for storing large nucleotide sequence alignments.\\
BAM: Binary representation of a SAM file, contains the same information but is compressed and indexed.\\
RPKM: Reads Per Kilobase per Million mapped reads, a crude measure for gene abundance and expression.
...
Lastly, we thank J. Lücking for providing the fermentation samples used for metagenome and metatranscroptome sequencing.
